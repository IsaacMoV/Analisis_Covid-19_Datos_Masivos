...
def _fit_arima_state(pdf: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        n_forecast = val_mask.sum() + test_mask.sum()
        if n_forecast == 0:
            return pd.DataFrame(columns=[f.name for f in _PRED_SCHEMA])

        forecast_result = arima_model.get_forecast(steps=n_forecast)
        predicted = forecast_result.predicted_mean
        ci        = forecast_result.conf_int(alpha=0.05)

        out_rows = pdf.loc[val_mask | test_mask].copy()
        out_rows["predicted"]  = np.maximum(predicted.values, 0)  # casos no pueden ser negativos
        out_rows["yhat_lower"] = np.maximum(ci.iloc[:, 0].values, 0)
        out_rows["yhat_upper"] = ci.iloc[:, 1].values
...
def run_arima(features_df: DataFrame) -> Dict:
    """
    Ajusta ARIMA(7,1,1) independiente por estado usando applyInPandas.

    La distribución via applyInPandas convierte el problema de ajustar
    ~56 modelos secuencialmente en un problema embarrassingly parallel:
    Spark asigna cada estado a un ejecutor disponible, logrando
    paralelismo real sin compartir estado entre modelos.
    """
    log.info("=" * 60)
    ...
    predictions_df = (
        features_df
        .select("state", "date", TARGET)
        .groupBy("state")
        .applyInPandas(_fit_arima_state, schema=_PRED_SCHEMA)
    )
...
def run_prophet(features_df: DataFrame) -> Dict:
    """
    Ajusta Prophet independiente por estado usando applyInPandas.
    Mismo patrón de distribución que ARIMA: embarrassingly parallel por estado.
    """
    log.info("=" * 60)
    ...
    predictions_df = (
        features_df
        .select("state", "date", TARGET,
                *[c for c in ["wave_intensity"] if c in features_df.columns])
        .groupBy("state")
        .applyInPandas(_fit_prophet_state, schema=prophet_schema)
    )
